...
# create a async_browser from import and get_tools from Playwright toolkit instance
# nest_asyncio.apply()
# async_browser = create_async_playwright_browser(headless=False)
# toolkit = PlayWrightBrowserToolkit(async_browser=async_browser)
# tools = toolkit.get_tools()

## use class manager to handle browser instance for error handling
class BrowserManager:
    async def init(self):
        ...
        context = await self.async_browser.new_context()
...
## A+D for NodeTool timeout
async def tools_with_timeout(State: State) -> Dict[str, Any]:
    try:
        tools = await browser_manager.get_tools()
        ...
        return {
            'messages': [AIMessage(content='Timeout! Browser restarted')],
            'user_input_needed': True
        }

# init worker and evaluator LLM
worker_llm = ChatOpenAI(model=LLM_MODEL_GPT_4O)
worker_llm_with_tools = worker_llm.bind_tools(tools) # tools here only contain playwright tool_kit
...
def worker(state: State) -> Dict[str, Any]:
    ...
    ## I think we should create a file to import these messages. for clean code
    ...
    messages = state['messages'] # should use .copy()避免修改到原始message
    ...
    # Invoke LLM
    response = worker_llm_with_tools.invoke(messages) # need try and catch
...
# build up the whole conversation and submit to evaluator
